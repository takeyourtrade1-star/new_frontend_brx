...
async function proxy(request: NextRequest, pathSegments: string[]) {
  if (!AUCTION_API_URL) {
    return NextResponse.json(
      { detail: 'Servizio ordini non disponibile' },
  ...
  if (!isAllowedAuctionProxyPath('orders', request.method, path)) {
  ...
  const rl = await checkRateLimit(request, { scope: 'orders', limit: 60, windowMs: 60_000, userId });
  ...
  try {
    ...
    if (!res.ok) return redactedUpstreamErrorResponse(res.status, 'Operazione ordine non riuscita');
