...
import { AsteScreen } from '@/components/feature/scambi/AsteScreen';
  ...
  return (
    <div className="fixed inset-0 z-[50] flex flex-col bg-black">
      ...
      <Link
        ...
        aria-label={t('scambi.backAria')}
        style={{ fontFamily: 'var(--font-comodo)' }}
      >
        <ArrowLeft className="h-5 w-5 shrink-0" strokeWidth={2.5} aria-hidden />
        <span>{t('scambi.backHome')}</span>
      </Link>

      {/* Bottone AVVISAMI + link di test in basso */}
      <div className="absolute inset-x-0 bottom-0 z-10 flex flex-col items-center gap-3 pb-10 md:pb-14">
        <div className="flex items-center gap-4">
          ...
          <button
            ...
            className="text-sm font-sans text-white/80 underline underline-offset-4 hover:text-white"
          >
            test
          </button>
        </div>
      ...
      {showTest && (
        <div className="fixed inset-0 z-[70]">
          <AsteScreen showAll={false} setShowAll={() => {}} onClose={() => setShowTest(false)} />
        </div>
      )}
    </div>
  );
