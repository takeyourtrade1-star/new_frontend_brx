  ...
  if (isAuthenticated && flashMessage) {
    return (
      <div className="space-y-4 text-center">
        <p className="text-gray-700">{t('accountBusiness.success')}</p>
        <Link
          href="/"
  ...
  return (
    <form onSubmit={handleSubmit(onSubmit)} className="space-y-4">
      <div className="absolute -left-[9999px] top-0 h-0 w-0 overflow-hidden" aria-hidden>
        <label htmlFor="website_url_business">{t('accountBusiness.honeypot')}</label>
        <input
      ...
      <div>
        <label className="mb-1 block text-sm font-medium text-gray-700">
          {t('accountBusiness.companyLabel')}
        </label>
        <Input
          {...register('ragione_sociale')}
          placeholder={t('accountBusiness.companyPlaceholder')}
      ...
      <div>
        <label className="mb-1 block text-sm font-medium text-gray-700">{t('accountBusiness.username')}</label>
        <Input
          {...register('username')}
      ...
      <div>
        <label className="mb-1 block text-sm font-medium text-gray-700">{t('accountBusiness.email')}</label>
        <Input
          {...register('email')}
          type="email"
      ...
      <div className="relative">
        <label className="mb-1 block text-sm font-medium text-gray-700">{t('accountBusiness.password')}</label>
        <Input
          {...register('password')}
          type={showPassword ? 'text' : 'password'}
          placeholder={t('accountBusiness.passwordHint')}
      ...
      <div className="flex gap-2">
        <div className="w-24 shrink-0">
          <label className="mb-1 block text-sm font-medium text-gray-700">{t('accountBusiness.phonePrefix')}</label>
          <Controller
            name="phone_prefix"
            control={control}
            render={({ field }) => (
        ...
        <div className="flex-1">
          <label className="mb-1 block text-sm font-medium text-gray-700">{t('accountBusiness.phone')}</label>
          <Input
            {...register('phone')}
            type="tel"
      ...
      <div>
        <label className="mb-1 block text-sm font-medium text-gray-700">{t('accountBusiness.country')}</label>
        <Controller
          name="country"
          control={control}
          render={({ field }) => (
      ...
      <div className="space-y-2">
        <label className="flex items-start gap-2">
          ...
          <span className="text-sm text-gray-700">{t('accountBusiness.terms')}</span>
        </label>
        <label className="flex items-start gap-2">
          <input type="checkbox" {...register('privacyAccepted')} className="mt-1" />
          <span className="text-sm text-gray-700">{t('accountBusiness.privacy')}</span>
        </label>
        <label className="flex items-start gap-2">
          <input type="checkbox" {...register('cancellationAccepted')} className="mt-1" />
          <span className="text-sm text-gray-700">{t('accountBusiness.cancellation')}</span>
        </label>
        <label className="flex items-start gap-2">
          <input type="checkbox" {...register('adultConfirmed')} className="mt-1" />
          <span className="text-sm text-gray-700">{t('accountBusiness.adult')}</span>
        </label>
      ...
      <div className="flex flex-col gap-3 pt-2 sm:flex-row sm:items-center">
        ...
        >
          {isLoading ? t('accountBusiness.submitting') : t('accountBusiness.submit')}
        </Button>
        <Link
          href="/registrati"
          className="text-center text-sm font-medium text-[#FF8C4B] hover:underline"
        >
          {t('accountBusiness.personalAccount')}
        </Link>
      </div>
    </form>
  );
